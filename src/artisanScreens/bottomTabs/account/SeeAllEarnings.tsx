...
import { artisanEarnings } from "../../../utils/dummyData";
...
const RenderItem = ({ item, earnings, index }) => {
  const navigation = useNavigation<any>();
  return (
    <Pressable
      key={index}
      style={{ gap: 15 }}
      onPress={() => navigation.navigate("PaymentInvoice", { data: item })}
    >
...
export const SeeAllEarnings = () => {
  const [earnings, setEarnings] = useState<Array<ArtisanEarnings>>([]);
  const modifiedData = artisanEarnings.sort((a, b) => b.date - a.date);
  const [dataLoading, setDataLoading] = useState(true);
  useEffect(() => {
    setEarnings(modifiedData);
    setTimeout(() => {
  ...
  return (
    ...
    >
      <OverviewPagesHeader title="My Earnings" hideRightComp />
      {dataLoading && (
        ...
        <>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <RenderItem item={item} index={index} earnings={earnings} />
            )}
